...
API = IQ_Option('watashisousa@hotmail.com', 'qaplty123')
API.connect()
...
def cataloga(par, dias, prct_call, prct_put, timeframe):
	...
	sair = False
	while sair == False:
		velas = API.get_candles('EURUSD', (timeframe * 60), 1000, time_)
		velas.reverse()
	...
	analise = {}
	for velas in data:
		horario = datetime.fromtimestamp(velas['from']).strftime('%H:%M:%S')
			...
			for horario in sorted(catalogacao[par]):
				if martingale.strip() != '':					
					...
					for i in range(int(martingale)):
						...
						mg_time = str(datetime.strptime((datetime.now()).strftime('%Y-%m-%d ') + str(mg_time), '%Y-%m-%d %H:%M:%S') + timedelta(minutes=timeframe))[11:]
...
for par in catalogacao:
	for horario in sorted(catalogacao[par]):
		...
		if ok == True:
			...
			msg = data2 + ',' +Fore.YELLOW + horario +  Fore.RESET + ',' + par + ',' + str(timeframe) +',' + (Fore.RED if catalogacao[par][horario]['dir'] == 'PUT ' else Fore.GREEN)  + catalogacao[par][horario]['dir'] + Fore.RESET + '\t' + str(catalogacao[par][horario]['%']) + '%  ' + Back.GREEN +  Fore.BLACK + str(catalogacao[par][horario]['verde']) + Back.RED + Fore.BLACK +  str(catalogacao[par][horario]['vermelha']) + Back.RESET +  Fore.RESET + str(catalogacao[par][horario]['doji'])
			...
			if martingale.strip() != '':
				for i in range(int(martingale)):
					if str(catalogacao[par][horario]['mg'+str(i+1)]['%']) != 0:
			...
			print(msg)	
			open('sinais_' + str((datetime.now()).strftime('%Y-%m-%d')) + '_' + str(timeframe) + 'M.txt', 'a').write(data2 + ','  + horario  + ',' + par + ',' + str(timeframe) +',' +  catalogacao[par][horario]['dir'] + ',&,' + '\t' + str(catalogacao[par][horario]['%']) + '%  '  + str(catalogacao[par][horario]['verde']) +  str(catalogacao[par][horario]['vermelha'])  +  str(catalogacao[par][horario]['doji'])+ '\n') #(horario + ',' + par + ',' + catalogacao[par][horario]['dir'].strip() + '\n')
